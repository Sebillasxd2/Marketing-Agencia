...
export default async function EditarContratoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <Link href={`/contratos/${c.id}`} className="text-sm text-gray-500 transition hover:text-gray-700">
        ← Volver al cliente
      </Link>
      ...
      {u.rol === 'jefa' && (
        <>
          <section className="rounded-2xl border border-gray-200 bg-white">
            ...
            {empleados.length === 0 ? (
              <p className="px-5 py-4 text-sm text-gray-400">Aún no tienes empleados. Créalos en la sección Trabajadores.</p>
            ) : (
